...
void test_wordlist_invalid_args(void) {
    ...
    assert(wordle_wordlist(buf, 6, "bre_d", "", "") == 0 && "Invalid letter required");
    assert(wordle_wordlist(buf, 6, NULL, "", "") == 0 && "Null required");

    assert(wordle_wordlist(buf, 6, "bread", "_", "") == 0 && "Invalid letter forbidden");
    assert(wordle_wordlist(buf, 6, "bread", "1", "") == 0 && "Invalid letter forbidden 2");
    assert(wordle_wordlist(buf, 6, "bread", NULL, "") == 0 && "Null forbidden");

    assert(wordle_wordlist(buf, 6, "bread", "", "_") == 0 && "Invalid letter placement");
    assert(wordle_wordlist(buf, 6, "bread", "", ":") == 0 && "Invalid letter placement 2");
    assert(wordle_wordlist(buf, 6, "bread", "", NULL) == 0 && "Null placement");
    assert(wordle_wordlist(buf, 6, "br-ad", "e9", NULL) == 0 && "Invalid digit");
    assert(wordle_wordlist(buf, 6, "br-ad", "e6", NULL) == 0 && "Invalid digit 2");
    assert(wordle_wordlist(buf, 6, "br-ad", "6", NULL) == 0 && "Missing char");
    assert(wordle_wordlist(buf, 6, "br-ad", "e0A9", NULL) == 0 && "Missing char 2");
}
